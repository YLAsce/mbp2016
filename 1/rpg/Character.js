...
Character.prototype.checkMap = function (dir){
	...
	switch (dir){
		case UP:
			if(self.y + charaLayer.y> 5*STEP)break;
			if(mapLayer1.y >= 0)break;
			addMap(0,-1);
			mapmove = true;
			break;
		case LEFT:
			if(self.x + charaLayer.x> 5*STEP)break;
			if(mapLayer1.x >= 0)break;
			addMap(-1,0);
			mapmove = true;
			break;
		case RIGHT:
			if(self.x + STEP + charaLayer.x< screen.width - 5*STEP)break;
			if(screen.width - mapLayer1.x >= map1[0].length*STEP)break;
			addMap(1,0);
			mapmove = true;
			break;
		case DOWN:
			if(self.y + STEP + charaLayer.y< screen.height - 5*STEP)break;
			if(screen.height - mapLayer1.y >= map1.length*STEP)break;
			addMap(0,1);
			mapmove = true;
			break;
	}
